...
exports.addLikesInTonesByUsers = function(user, tones, fn){
  console.log('[controller/Like] addLikesInTonesByUsers:'+tones.length);
  var tones_ids = [];
  for (var i=0; i<tones.length;i++){
    tones_ids.push(tones[i].id+'');
    console.log('[controller/Like] addLikesInTonesByUsers:'+i+'='+tones[i].id);
  }
  ToneLike.find({'user_id':user._id}).where('tone_id').in(tones_ids).exec(function(err, tonelikes){
  	if(err){
  	  fn(err, {"error":"[like] getLikesByUsersAndItemTones: An error has occurred"});
          ...
          console.log("[like] tones:"+tones.length);
	    for (var i=0; i<tones.length;i++){
	      tones[i].like = false;
	      for (var j=0; j<tonelikes.length;j++){
          console.log("[like] addLikesInTonesByUsers("+tones[i]._id+","+tonelikes[j].tone_id+")");
...
exports.getLikesByUser = function (user, fn) {
  ToneLike.find({'user_id':user._id}).exec(function(err, tonelikes){
    if(err){
      ...
      var tones_ids = [];
      console.log("[like] tonelikes:"+tonelikes.length);
...
exports.likeTone = function (user_id, tone_id, fn) { 
  console.log('[controller/Like] likeTone:'+user_id+','+tone_id);
  ToneLike.find({'user_id':user_id,'tone_id':tone_id}).exec(function(err, tonelikes){
    if(tonelikes.length>0){
      console.log('[controller/Like] likeTone tonelikes.length>0:'+user_id+','+tone_id);
      ...
      console.log('[controller/Like] likeTone save:'+user_id+','+tone_id);
